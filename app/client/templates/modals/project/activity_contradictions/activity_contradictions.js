...
Template.ActivityContradictions.events({
    ...
    'click #delete-contradiction-button': function(event, template) {
        event.preventDefault();
        Meteor.call('deleteContradiction', Session.get('contradictionToDeleteData'), Contradictions.findOne({ _id: Session.get('contradictionToDeleteData') }).projectId, function(error, result) {
            if (error) {
                var errorNotice = new PNotify({
                    type: 'error',
                    title: 'Error',
...
Template.ActivityContradictions.helpers({
    ...
    deleteContradictionData: function() {
        Session.get('contradictionToDeleteData');
    },
    showContradictionData: function() {
        return Contradictions.findOne({ _id: Session.get('contradictionToShowData') });
    },
    editContradictionData: function() {
        return Session.get('contradictionToEditData');
    },
});
